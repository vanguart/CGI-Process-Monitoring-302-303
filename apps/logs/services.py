...
def transformLog(creds,header,keys):
    ...
    for key in newKeys:
        ...
        logsTrace = requests.get(creds['url'] + f"/odata/RobotLogs?$filter=JobKey eq {key} and Level eq 'Trace'", headers=header)
        ...
        for logTrace in range(0,countLogsTrace):
            ...
            if jsonSplitMessageLog[0] == 'Process':
                ...
                newOrOldProcessConfiguration,newQueueProcess = loadLogProcess(processType,processName,description,maxTimeOfExecution,skills)
        ...
        if newOrOldProcessConfiguration == None:
            return
        
        # There was no log Process so nothing is going to be added in your database
        if newQueueProcess == None:
            return  
        

        logsError = requests.get(creds['url'] + f"/odata/RobotLogs?$filter=JobKey eq {key} and Level eq 'Error'", headers=header)
...
def loadLogTaskConfiguration(processConfiguration,taskType,taskName,description,maxTimeOfExecution,queueProcess,priority):
